...
import { removeFootprintAiTrigger, shouldAutoGenerateFootprintAi } from '../utils/footprintAiTrigger'
...
function MyFootprintPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !shouldAutoGenerateFootprintAi(location.search) ||
      loading ||
      error ||
    ...
    navigate(
      { pathname: location.pathname, search: removeFootprintAiTrigger(location.search) },
      { replace: true }
    )
    void handleGenerateAiSummary()
  }, [currentRange.begin, currentRange.end, error, handleGenerateAiSummary, loadedRangeKey, loading, location.pathname, location.search, navigate])
