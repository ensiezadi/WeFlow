 ...
 *
 * 设计原则：
 * - 不引入额外 npm 依赖，使用 Electron Chromium 网络栈调用 OpenAI 兼容 API并遵循系统代理
 * - 所有失败静默处理，不影响主流程
...
import { requestText } from './aiHttpClient'
import {
...
function isMiniMaxTarget(apiBaseUrl: string, model: string): boolean {
  const target = `${apiBaseUrl} ${model}`.toLowerCase()
  return target.includes('minimax') || target.includes('minimaxi.com')
}
...
async function callApi(
  ...
  try {
    ...
    throw new Error(`无效的 API URL: ${endpoint}`)
  }

    const miniMaxTarget = isMiniMaxTarget(apiBaseUrl, model)
    const normalizedMaxTokens = miniMaxTarget
      ? Math.min(2048, normalizeApiMaxTokens(maxTokens))
      : normalizeApiMaxTokens(maxTokens)
    ...
    if (options.useMaxCompletionTokens || miniMaxTarget) {
  ...
  if (response.statusCode >= 400) {
    throw new ApiRequestError(`API 请求失败 (${response.statusCode}): ${data.slice(0, 200)}`, response.statusCode, data)
  }
  ...
  try {
    ...
    throw new Error(`JSON 解析失败: ${data.slice(0, 200)}`)
  }
  ...
  if (typeof content === 'string' && content.trim()) {
    ...
    throw new Error('API 仅返回推理内容未返回正文，请增大最大输出 Token')
  }

  const finishReason = parsed?.choices?.[0]?.finish_reason
  const reasoningContent = parsed?.choices?.[0]?.message?.reasoning_content
  if (typeof reasoningContent === 'string' && reasoningContent.trim()) {
    throw new Error(`API 仅返回推理内容未返回正文${finishReason ? `（finish_reason=${finishReason}）` : ''}，请增大最大输出 Token 或关闭思考模式`)
  }
  throw new Error(`API 返回格式异常${finishReason ? `（finish_reason=${finishReason}）` : ''}: ${data.slice(0, 200)}`)
}
